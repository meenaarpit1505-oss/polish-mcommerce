...
export function RegisterForm({ locale }: RegisterFormProps) {
  ...
  const isFormValid = fullName.trim() !== "" && email.trim() !== "" && isPasswordValid && acceptTerms;
  ...
  const handleGoogleLogin = async () => {
    ...
    if (!isSupabaseConfigured) {
      setError(locale === "pl" ? "Rejestracja przez Google jest wyłączona w wersji demonstracyjnej bez bazy danych Supabase." : "Google Registration is disabled in the demo version without a Supabase database.");
      return;
    }
  ...
  return (
    <div className="grid min-h-[calc(100vh-64px)] w-full grid-cols-1 md:grid-cols-12 bg-[#F8FAFC] dark:bg-[#0F172A]">
      ...
      {/* RIGHT COLUMN: Sign Up Form & Success Screen */}
            ...
            {!isSuccess ? (
              ...
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="flex items-start gap-3 rounded-xl bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 p-4 text-red-800 dark:text-red-300">
                          ...
                          <div className="space-y-1 text-left text-xs sm:text-sm">
                            ...
                            <p className="text-red-700/90 dark:text-red-400/90 leading-relaxed">
                              {error === "already_exists"
                                ? t("alreadyExistsSubtitle")
                                : error === "weak_password"
                                ? t("weakPasswordSubtitle")
                                : error === "internal_error"
                                ? t("genericErrorSubtitle")
                                : error}
                            </p>
                            {error === "already_exists" && (
                  ...
                  {/* Password Strength Checklist (Goal-Gradient Effect) */}
                  {password.length > 0 && (
                    <motion.div 
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: "auto" }}
                      className="rounded-xl bg-slate-50 dark:bg-slate-800/50 p-4 border border-slate-100 dark:border-slate-800/80 space-y-2.5"
                    >
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          <span className={`flex h-4 w-4 items-center justify-center rounded-full transition-all duration-300 ${
                            hasMinLength 
                          ...
                          <span className={hasMinLength ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-slate-500 dark:text-slate-400"}>
                            {t("reqLength")}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className={hasNumber ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-slate-500 dark:text-slate-400"}>
                            {t("reqNumber")}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          <span className={`flex h-4 w-4 items-center justify-center rounded-full transition-all duration-300 ${
                            ...
                            {hasSpecial ? <Check className="h-2.5 w-2.5 stroke-3" /> : <X className="h-2.5 w-2.5" />}
                          </span>
                          <span className={hasSpecial ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-slate-500 dark:text-slate-400"}>
                            {t("reqSpecial")}
                          </span>
                        </div>
                      </div>
                  ...
                  {/* Consents (Legal Transparency EU/UOKiK) */}
                  <div className="space-y-3.5 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <div className="relative flex items-center mt-0.5">
                        ...
                        />
                        <div className="h-4.5 w-4.5 rounded border border-slate-300 dark:border-slate-600 bg-transparent transition-all peer-checked:bg-emerald-500 peer-checked:border-emerald-500 group-hover:border-emerald-500 flex items-center justify-center">
                          <Check className="h-3 w-3 text-white stroke-3 opacity-0 peer-checked:opacity-100 transition-opacity" />
                        </div>
                      </div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 select-none leading-relaxed">
                        {t.rich("termsAccept", {
                          terms: (
                            <Link key="terms" href="/regulamin" className="text-emerald-600 dark:text-emerald-400 hover:underline font-medium">
                              {tLegal("terms")}
                            </Link>
                          ),
                          privacy: (
                            <Link key="privacy" href="/polityka-prywatnosci" className="text-emerald-600 dark:text-emerald-400 hover:underline font-medium">
                              {tLegal("privacy")}
                            </Link>
                          ),
                        })}
                  ...
                  {/* Submit Button (Goal-Gradient Effect & Success Glow) */}
                  <button
                    type="submit"
              ...
              /* SUCCESS SCREEN (Dopamine Loop Micro-Reward) */
              <motion.div
                key="success-screen"
