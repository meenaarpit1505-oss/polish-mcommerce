...
export function PromoSectionBlock({ promo, products, categories }: PromoSectionProps) {
  ...
  // Filter products: if category is active, show all in that category.
  // If no category is active, show only promo/low-stock products (limited to top 3).
  ...
  // If there are no products to display, hide the section gracefully
    ...
    <section className="relative bg-linear-to-br from-primary/5 to-accent/5 py-8 sm:py-10 transition-colors duration-300">
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex flex-col gap-2.5 items-start">
            <div className="flex flex-wrap items-center gap-3">
              <h2 className="text-xl font-bold text-foreground sm:text-2xl">
                {selectedCategoryObj ? selectedCategoryObj.name : promo.title || t("title")}
              </h2>
              {activeCategory && (
                <button
                  type="button"
          ...
          {/* Only show countdown when no category is active */}
          {!activeCategory && <Countdown endsAt={promo.endsAt} />}
        </div>
